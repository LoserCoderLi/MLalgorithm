...
class TextHARNN(object):
    ...
    '''
    它在创建类的实例时被调用。这个类似乎是一个用于训练神经网络模型的类，具体来说，可能是一个使用 LSTM 和注意力机制的模型。
    这是类的初始化方法的定义。它接受多个参数，包括序列长度、词汇表大小、嵌入类型、嵌入大小、LSTM隐藏层大小、注意力单元大小、全连接层隐藏层大小、类别列表、总类别数、L2正则化系数和预训练的嵌入。
    '''
    def __init__(
        ...
        def _attention(input_x, num_classes, name=""):
            """
            用于计算注意力权重并生成注意力模型的输出。
            这个函数的主要目的是通过计算输入和注意力权重的加权和来提取输入序列中的重要信息。
            Attention Layer.
        ...
        def _fc_layer(input_x, name=""):
            """
            全连接层的主要作用是学习输入特征的非线性组合，以此提高模型的表达能力。
        ...
        def _local_layer(input_x, input_att_weight, num_classes, name=""):
            """
            计算了模型的输出分数和 logits，并生成了一个可视化的注意力权重，这可以帮助我们理解模型的注意力机制。
            Local Layer.
        ...
        def _linear(input_, output_size, initializer=None, scope="SimpleLinear"):
            """
            实现了一个线性映射，将输入数据映射到一个新的向量空间，这个新的向量空间的维度由 output_size 指定。
            线性映射是一种简单的数学操作，用于将输入向量转换到新的向量空间。
            在神经网络中，线性映射通常用于将输入数据转换到新的特征空间，或者用于在网络的最后一层生成预测。
        ...
        def _highway_layer(input_, size, num_layers=1, bias=-2.0):
            """
            这个函数实现了一个高速网络，
            它通过引入门控机制，使得部分信息可以直接通过网络，
            从而缓解了深层网络训练中的梯度消失和爆炸问题。
        ...
        '''
        这段代码定义了一个深度学习模型的构建过程，
        该模型结合了词嵌入（Embedding）、双向长短期记忆网络（Bi-LSTM）、
        注意力机制（Attention）、全连接层（Fully Connected Layer）、
        高速网络（Highway Network）和丢弃层（Dropout）。
        这个模型的主要目的是进行多级别的文本分类（Hierarchical Multi-Label Text Classification）。
        '''
        # Embedding Layer
