...
'''
作用是维护一个只包含最佳n个检查点的目录。
目录内包含一个名为best_checkpoints的JSON文件，
该文件包含一个字典，字典的键是最佳检查点文件的路径，值是对应的评价指标。
只有最佳的n个检查点会被保存在目录和JSON文件中。
'''
class BestCheckpointSaver(object):
    ...
    """
    '''
    初始化方法，设置保存目录，要保留的检查点数量，
    定义"最佳"值的方式（最大值或最小值），并设置一个tf.train.Saver实例。
    '''
    ...
    '''
    根据给定的结果更新最佳检查点集。
    如果当前检查点比现有的最佳检查点更好（或者现有的最佳检查点数量还没达到要保留的数量），
    那么就将当前检查点保存下来。
    '''
...
'''
这个函数的作用是从指定的目录中找出并返回最佳的检查点文件路径。
这个"最佳"的定义取决于select_maximum_value参数。
如果select_maximum_value为True，那么函数会返回与最高值相关联的文件路径；
如果为False，那么函数会返回与最低值相关联的文件路径。
'''
